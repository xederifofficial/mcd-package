...
COPY --from=mc-builder /mc-build/mobilecoin/target/release/mobilecoind /usr/local/bin/mobilecoind
COPY --from=mc-builder /mc-build/mobilecoin/target/release/mc-admin-http-gateway /usr/local/bin/mc-admin-http-gateway
COPY --from=mc-builder /mc-build/mobilecoin/consensus-enclave.css /usr/local/bin/consensus-enclave.css
COPY --from=mc-builder /mc-build/mobilecoin/ingest-enclave.css /usr/local/bin/ingest-enclave.css
